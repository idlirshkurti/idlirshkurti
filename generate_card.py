...
# Public API summaries: repo count, stars, followers and current tracked source footprint.
# Commit/LOC counters can be extended later with a personal token if full-history private data is desired.
...
def card(path,bg,fg,muted):
 ...
 svg=f'''<svg xmlns="http://www.w3.org/2000/svg" width="1350" height="700" viewBox="0 0 1350 700"><style>.t{{font:700 34px monospace;fill:{fg}}}.s,.l{{font:18px monospace;fill:{muted}}}.v{{font:700 18px monospace;fill:{fg}}}.a{{font:10px monospace;fill:{fg}}}</style><rect width="1350" height="700" rx="22" fill="{bg}"/><rect x="24" y="24" width="1302" height="652" rx="15" fill="none" stroke="{muted}" stroke-opacity=".5"/><line x1="620" y1="48" x2="620" y2="652" stroke="{muted}" stroke-opacity=".5"/><text x="42" y="85" class="t">idlirshkurti@github</text>{stats}<text class="a" xml:space="preserve">{portrait}</text><text x="660" y="650" class="s">ASCII profile portrait</text></svg>'''
